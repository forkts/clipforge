...
import { ChevronLeft, ChevronRight, Music, Pause, Play, Sparkles, X } from 'lucide-react';
...
interface FullscreenViewerProps {
  isOpen: boolean;
  onClose: () => void;
  items: Array<{ url: string; type: FileType }>;
  initialIndex: number;
...
export const FullscreenViewer = ({
  ...
  const isVideo = currentItem.type === FileType.Video;
  const isAudio = currentItem.type === FileType.Audio;
  const isImage = currentItem.type === FileType.Image;
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentItem.url &&
            (isVideo ? (
              <video src={currentItem.url} className="h-full w-full object-cover" />
            ) : isImage ? (
              <img src={currentItem.url} alt="" className="h-full w-full object-cover" />
            ) : (
              <audio src={currentItem.url} className="h-full w-full object-cover" />
            ))}
        </motion.div>
      </AnimatePresence>
      ...
      {currentItem.url ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {isVideo ? (
                ...
                <div className="flex flex-col items-center justify-center w-full h-full bg-black/20 rounded-3xl p-8 max-w-2xl border border-white/5">
                  <div className="w-32 h-32 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-500 mb-8 shadow-[0_0_50px_-10px_rgba(16,185,129,0.3)] border border-emerald-500/30">
        ...
        <div className="flex flex-col items-center gap-4 text-white/20">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em]">URL is missing</p>
        </div>
      )}
      ...
      {items.length > 1 && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-[110] flex gap-3 p-3 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-3xl max-w-[90vw] overflow-x-auto no-scrollbar pointer-events-auto">
          {items.map((item, idx) => (
            ...
            >
              {item.url && item.type !== FileType.Audio && (
